from rail_walker_interface import JoystickPolicy, JoystickEnvironment, BaseWalker, BaseWalkerInSim, \
    ReadOnlyWalkerWrapper, JoystickEnvImpl
from typing import Tuple
import gym
import gym.wrappers
from rail_walker_gym.envs.wrappers.relabel_wrapper import RelabelAggregateWrapper, RelabelTargetProvider, \
    SymmetricRelabelWrapper
...
from rail_walker_gym.joystick_policy import JoystickPolicyStrictRewardProvider, JoystickPolicyForwardOnlyTargetProvider, \
    JoystickPolicyResetterEarlyStopTruncationProvider, JoystickPolicyManualResetter
from rail_walker_gym.joystick_policy_mujoco import JoystickPolicyPointInSimResetter, \
    JoystickPolicyLastPositionAndYawResetter
...
def _helper_apply_resetter_wrapper(
    ...
    for resetter in joystick_policy.resetters:
        if isinstance(resetter, JoystickPolicyPointInSimResetter) or isinstance(resetter,
                                                                                JoystickPolicyLastPositionAndYawResetter):
...
def _apply_task_configs(
    ...
    if not joystick_policy.target_yaw_provider.is_target_velocity_fixed():
        task_suffix += "_tvdyn"
        if hasattr(joystick_policy.target_yaw_provider, "min_target_velocity") and hasattr(
                joystick_policy.target_yaw_provider, "max_target_velocity"):
        ...
        if hasattr(joystick_policy.target_yaw_provider, "max_velocity_change_rate"):
            setattr(joystick_policy.target_yaw_provider, "max_velocity_change_rate",
                    task_configs.max_velocity_change_rate)
            task_suffix += "-{:.2f}".format(task_configs.max_velocity_change_rate)
    else:
        task_suffix += "_tv{:.2f}".format(
            joystick_policy.target_yaw_provider.get_target_velocity(joystick_policy.robot))

    if hasattr(joystick_policy.target_yaw_provider, "keep_target_delta_yaw_steps_avg") and hasattr(
            joystick_policy.target_yaw_provider, "keep_velocity_steps_avg"):
        setattr(joystick_policy.target_yaw_provider, "keep_target_delta_yaw_steps_avg",
                task_configs.joystick_keep_target_delta_yaw_steps_avg)
        setattr(joystick_policy.target_yaw_provider, "keep_velocity_steps_avg",
                task_configs.joystick_keep_target_velocity_steps_avg)
        task_suffix += "_keep,yaw{:d},tv{:d}".format(task_configs.joystick_keep_target_delta_yaw_steps_avg,
                                                     task_configs.joystick_keep_target_velocity_steps_avg)
    ...
    if (
        ...
        setattr(joystick_policy.reward_provider, "pitch_rate_penalty_factor",
                task_configs.rew_pitch_rate_penalty_factor)
        setattr(joystick_policy.reward_provider, "roll_rate_penalty_factor", task_configs.rew_roll_rate_penalty_factor)
        task_suffix += "_avpf{:.2f},{:.2f}".format(task_configs.rew_pitch_rate_penalty_factor,
                                                   task_configs.rew_roll_rate_penalty_factor)
    ...
    if (
        ...
        setattr(joystick_policy.reward_provider, "joint_diagonal_penalty_weight",
                task_configs.rew_joint_diagonal_penalty_weight)
        setattr(joystick_policy.reward_provider, "joint_shoulder_penalty_weight",
                task_configs.rew_joint_shoulder_penalty_weight)
        task_suffix += "_jspw{:.2e},{:.2e}".format(task_configs.rew_joint_diagonal_penalty_weight,
                                                   task_configs.rew_joint_shoulder_penalty_weight)

    if (
            hasattr(joystick_policy.reward_provider, "joint_acc_penalty_weight") and
            hasattr(joystick_policy.reward_provider, "joint_vel_penalty_weight")
    ):
        ...
        task_suffix += "_qpw{:.2e},{:.2e}".format(task_configs.rew_joint_acc_penalty_weight,
                                                  task_configs.rew_joint_vel_penalty_weight)
    ...
    if hasattr(joystick_policy.reward_provider, "energy_penalty_weight"):
        ...
        task_suffix += "_ep{:.3f}".format(
            task_configs.rew_energy_penalty_weight) if task_configs.rew_energy_penalty_weight > 0.0 else "_nep"
    ...
    if hasattr(joystick_policy.reward_provider, "smooth_torque_penalty_weight"):
        setattr(joystick_policy.reward_provider, "smooth_torque_penalty_weight",
                task_configs.rew_smooth_torque_penalty_weight)
        task_suffix += "_stpw{:.2e}".format(task_configs.rew_smooth_torque_penalty_weight)

    if hasattr(joystick_policy.reward_provider, "modification"):
        setattr(joystick_policy.reward_provider, "modification", task_configs.modification)
        task_suffix += f"_mod_{task_configs.modification}"
