...
class Dataset(object):
    def __init__(self, dataset_dict: DatasetDict, tsteps=0, seed: Optional[int] = None):
    ...
    def sample(
        ...
        if indx is None:
            if hasattr(self.np_random, "integers"):
                indx = self.np_random.integers(self.tsteps, len(self), size=batch_size)
            else:
                indx = self.np_random.randint(self.tsteps, len(self), size=batch_size)
        ...
        for k in keys:
            if isinstance(self.dataset_dict[k], dict):
                ...
                if self.tsteps > 0:
                    if k == 'next_observations' or k == 'observations':
                        tmp_list = [batch[k]]
                        for t in range(1, self.tsteps + 1):
                            tmp_list.insert(0, self.dataset_dict[k][indx - t])
                        batch[k] = np.concatenate(tmp_list, 1)
    ...
    def sample_last_k(
        ...
        if indx is None:
            if hasattr(self.np_random, "integers"):
                indx = self.np_random.integers(len(self) - k if len(self) - k >= 0 else 0, len(self), size=batch_size)
            else:
                indx = self.np_random.randint(len(self) - k if len(self) - k >= 0 else 0, len(self), size=batch_size)
