...
class ActionFilterHelper(object):
    ...
    def reset(self):
        self._action_filter.init_history(self._init_qpos)
...
class ActionFilterWrapper(gym.ActionWrapper):
    ...
    def __init__(self, env : gym.Env, control_timestep : float, walker : BaseWalker, highcut: int = 4):
        super().__init__(env)
        self._action_filter_helper = ActionFilterHelper(control_timestep, walker.joint_nums, walker.joint_qpos_init, highcut)

    def reset(self, *args, seed: "int | None = None", options: "dict[str, Any] | None = None", **kwargs):
        self._action_filter_helper.reset()
        return self.env.reset(*args, seed=seed, options=options, **kwargs)

    def action(self, action):
        action = self._action_filter_helper.filter(action)
        return action