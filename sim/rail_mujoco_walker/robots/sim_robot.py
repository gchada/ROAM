...
class DMWalkerForRailSimWalkerObservables(composer.Observables): #(base.WalkerObservables):
    def __init__(self, entity : "DMWalkerForRailSimWalker", foot_contact_threshold : np.ndarray, delayed_substep : int = 5):
    ...
    def foot_forces_normalized(self):
        ...
        return self._entity.get_delayed_observable(
            ...
            delay_substeps=self._delayed_substep,
            lambda_fn=lambda physics: (physics.bind(self._entity.mjcf_model.sensor.touch).sensordata / foot_force_max_values).astype(np.float32)
        )
...
class RailSimWalkerDMControl(BaseWalker[typing.Dict[str,np.ndarray]],BaseWalkerInSim, BaseWalkerWithFootContact):
    def __init__(
        ...
        foot_contact_threshold : np.ndarray = np.array([10.0,10.0,10.0,10.0]),
        *args, 
        **kwargs
    ):
