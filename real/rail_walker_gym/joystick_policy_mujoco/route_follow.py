...
class JoystickPolicyRouteFollow2DTraversibleSimRouteResetter(JoystickPolicyResetter[BaseWalkerInSim]):
    ...
    def perform_reset(
        ...
        if not self._inited or self.traversible.is_complete or self.traversible.robot_stuck:
            if self.traversible.robot_stuck:
                print("JoystickPolicyRouteFollow2DTraversibleSimRouteResetter: Robot stuck for 5 minutes, respawning to start.")
