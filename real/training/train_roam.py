...
def main(_):
    ...
    agent_0_path = "walk_init/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly/agent0"
    agent_1_path = "joint_2/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly/agent0"
    agent_2_path = "joint_5/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly/agent0"
    agent_3_path = "joint_8/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly/agent0"
    agent_4_path = "joint_11/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly/agent0"
    ...
    for i in range(NUM_AGENTS):
        ...
        for agent_i in range(FLAGS.num_agents):
            agent_i_loaded_checkpoint_step, agents[agent_i] = load_latest_checkpoint(agent_paths[i], agents[agent_i], 0)
        all_agents.append(agents)

    buffer_0_path = "walk_init/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly"
    buffer_1_path = "joint_2/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly"
    buffer_2_path = "joint_5/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly"
    buffer_3_path = "joint_8/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly"
    buffer_4_path = "joint_11/saved_0912_indoor_reset/Go1SanityReal-Empty-SepRew-v0_s0_maxgr100.00_ac60000_1.0to1.0_linearly"
    buffer_paths = [buffer_0_path, buffer_1_path, buffer_2_path, buffer_3_path, buffer_4_path]

    for i in range(NUM_AGENTS):
        rb_load = load_latest_replay_buffer(buffer_paths[i])
        replay_buffer_loaded_step, replay_buffer = rb_load
        offline_buffers_raw.append(replay_buffer)
    
    offline_buffer_0 = ReplayBuffer(env.observation_space, env.action_space, len(offline_buffers_raw[0]))
    offline_buffer_0.seed(FLAGS.seed)

    offline_buffer_1 = ReplayBuffer(env.observation_space, env.action_space, len(offline_buffers_raw[1]))
    offline_buffer_1.seed(FLAGS.seed)

    offline_buffer_2 = ReplayBuffer(env.observation_space, env.action_space, len(offline_buffers_raw[2]))
    offline_buffer_2.seed(FLAGS.seed)

    offline_buffer_3 = ReplayBuffer(env.observation_space, env.action_space, len(offline_buffers_raw[3]))
    offline_buffer_3.seed(FLAGS.seed)

    offline_buffer_4 = ReplayBuffer(env.observation_space, env.action_space, len(offline_buffers_raw[4]))
    offline_buffer_4.seed(FLAGS.seed)


    offline_buffer_0.add(offline_buffers_raw[0], len(offline_buffers_raw[0]) - 3000, 0)
    offline_buffer_1.add(offline_buffers_raw[1], len(offline_buffers_raw[1]) - 3000, 1)
    offline_buffer_2.add(offline_buffers_raw[2], len(offline_buffers_raw[2]) - 3000, 2)
    offline_buffer_3.add(offline_buffers_raw[3], len(offline_buffers_raw[3]) - 3000, 3)
    offline_buffer_4.add(offline_buffers_raw[4], len(offline_buffers_raw[4]) - 3000, 4)


    offline_buffers = [offline_buffer_0, offline_buffer_1, offline_buffer_2, offline_buffer_3, offline_buffer_4]


    agent_i_loaded_checkpoint_step, replay_buffer_loaded_step = 0, 0

    online_buffer_0 = ReplayBuffer(env.observation_space, env.action_space, FLAGS.max_steps)
    online_buffer_0.seed(FLAGS.seed)

    online_buffer_1 = ReplayBuffer(env.observation_space, env.action_space, FLAGS.max_steps)
    online_buffer_1.seed(FLAGS.seed)

    online_buffer_2 = ReplayBuffer(env.observation_space, env.action_space, FLAGS.max_steps)
    online_buffer_2.seed(FLAGS.seed)

    online_buffer_3 = ReplayBuffer(env.observation_space, env.action_space, FLAGS.max_steps)
    online_buffer_3.seed(FLAGS.seed)

    online_buffer_4 = ReplayBuffer(env.observation_space, env.action_space, FLAGS.max_steps)
    online_buffer_4.seed(FLAGS.seed)

    online_buffers = [online_buffer_0, online_buffer_1, online_buffer_2, online_buffer_3, online_buffer_4]

    PRETRAIN_STEPS = 3000
    joystick_policy.robot.hibernate()

    for i in tqdm.tqdm(range(PRETRAIN_STEPS)):
        ...
        agent = agents_for_update[buffer_num]
        grads, info = agent.update_offline(batch, 1, labels, agents_for_update, buffer_num)

        for j, agent_j in enumerate(agents_for_update):

            critic = agent_j.critic.apply_gradients(grads=grads[j + 1])
            if j == buffer_num:
                critic = critic.apply_gradients(grads=grads[0])

            target_critic_params = optax.incremental_update(
                critic.params, agent_j.target_critic.params, agent_j.tau
            )
            target_critic = agent_j.target_critic.replace(params=target_critic_params)
            agent_j = agent_j.replace(critic=critic, target_critic=target_critic)
            agents_for_update[j] = agent_j
    ...
    max_agent = 0
    agents = all_agents[max_agent]
    replay_buffer = online_buffers[max_agent]
    agent_loaded_checkpoint_step, replay_buffer_loaded_step = 0, 0
    ...
    try:
        for i in tqdm.trange(agent_loaded_checkpoint_step, FLAGS.max_steps + agent_loaded_checkpoint_step, initial=agent_loaded_checkpoint_step, disable=not FLAGS.tqdm, smoothing=0.1):
            ...
            if i >= FLAGS.start_training + agent_loaded_checkpoint_step - replay_buffer_loaded_step and not jax_jitted_rb:
                ...
                for num_agent in range(NUM_AGENTS):
                    agents = all_agents[num_agent]
                    replay_buffer = online_buffers[num_agent]
            ...
            if i < FLAGS.start_training:
                ...
                vs = []
                for pos in range(len(all_agents)):
                    agent = all_agents[pos][0]
                    agent_qs = []
            ...
            if joystick_policy is not None and joystick_policy.robot.is_real_robot and ((
                ...
                if i < FLAGS.start_training:
                    for buf in online_buffers:
                        buf.insert(transition_dict)
                else:    
