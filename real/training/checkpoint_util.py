...
def initialize_project_log(project_dir, num_agents) -> None:
    os.makedirs(project_dir, exist_ok=True)
    
    agent_dirs = [os.path.join(project_dir, f"agent{i}") for i in range(num_agents)]
    for agent_dir in agent_dirs:
        os.makedirs(agent_dir, exist_ok=True)
        chkpt_dir = os.path.join(agent_dir, 'checkpoints')
...
def load_latest_checkpoint(project_dir, agent : JaxRLAgent, if_failled_return_step: int = 0) -> typing.Tuple[int, JaxRLAgent]:
    ...
    if len(chkpts) == 0:
        ...
        chkpt_int = int(chkpts[-1].split('_')[-1].split('.')[0])
