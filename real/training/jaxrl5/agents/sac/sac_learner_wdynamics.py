...
class SACLearnerWithDynamics(Agent):
    ...
    def update_critic_offline(self, batch: DatasetDict, labels: jnp.ndarray, agents, agent_pos) -> Tuple[TrainState, Dict[str, float]]:
        ...
        def critic_loss_fn(critic_params, critic_params_0, critic_params_1, critic_params_2, critic_params_3, critic_params_4):
            critic_params_arr = [critic_params_0, critic_params_1, critic_params_2, critic_params_3, critic_params_4]
            ...
            critic_loss = bellman_loss + 100 * ce_loss
            return critic_loss, {"ce_loss": ce_loss, "bellman_loss": bellman_loss}

        critic_params_arr = []
        for agent in agents:
            critic_params_arr.append(agent.critic.params)
        grads, info = jax.grad(critic_loss_fn, has_aux=True, argnums=[0, 1, 2, 3, 4, 5])(self.critic.params, *critic_params_arr)

        # for i, agent in enumerate(agents):
        #     critic = agent.critic.apply_gradients(grads=grads[i + 1])
        #     target_critic_params = optax.incremental_update(
        ...
        #
        #
        # critic = self.critic.apply_gradients(grads=grads[0])
        #
        # target_critic_params = optax.incremental_update(
        #     critic.params, self.target_critic.params, self.tau
        # )
    ...
    @partial(jax.jit, static_argnames="utd_ratio")
    def update_offline(self, batch: DatasetDict, utd_ratio: int, labels: jnp.ndarray, agents, pos):
        ...
        for i in range(utd_ratio):
            ...
            mini_batch = jax.tree_util.tree_map(slice, batch)
            grads, critic_info = new_agent.update_critic_offline(mini_batch, labels, agents, pos)

        return grads, {**critic_info}
