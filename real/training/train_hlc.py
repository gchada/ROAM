...
def main(_):
    ...
    full_offline_buffer = ReplayBuffer(env.observation_space, env.action_space, total_steps)
    full_offline_buffer.seed(FLAGS.seed)

    full_offline_buffer.add(offline_buffers_raw[0], len(offline_buffers_raw[0]) - 3000, 0)
    full_offline_buffer.add(offline_buffers_raw[1], len(offline_buffers_raw[1]) - 3000, 1)
    full_offline_buffer.add(offline_buffers_raw[2], len(offline_buffers_raw[2]) - 3000, 2)
    full_offline_buffer.add(offline_buffers_raw[3], len(offline_buffers_raw[3]) - 3000, 3)
    full_offline_buffer.add(offline_buffers_raw[4], len(offline_buffers_raw[4]) - 3000, 4)
    ...
    def update_state_network(state_network, batch):
        ...
        def loss_fn(state_net_params):
            ...
            categorical_labels = to_categorical(batch["labels"], num_classes=NUM_AGENTS)
